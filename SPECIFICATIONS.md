...
- [Overview](#overview)
- [Contracts](#contracts)
    + [KittyswapsExchange.sol](#kittyswapsexchangesol)
    + [KittyswapsFactory.sol](#kittyswapsfactorysol)
...
This document outlines the core mechanics and technical details for Kittyswaps. 

# Contracts

### KittyswapsExchange.sol

This contract is responsible for permitting the exchange between a single currency and all tokens in a given ERC-1155 token contract. For each token id $i$, the KittyswapsExchance contract holds a reserve of currency and a reserve of token id $i$, which are used to calculate the price of that token id $i$ denominated in the  currency. 

### KittyswapsFactory.sol

This contract is used to deploy a new KittyswapsExchange.sol contract for ERC-1155 contracts without one yet. It will keep a mapping of each ERC-1155 token contract address with their corresponding KittyswapsExchange.sol contract address.
...
The first 4 bytes of the `_data` argument indicate which of the four main [KittyswapsExchange.sol](https://github.com/kittyswaps/kittyswaps/blob/master/contracts/exchange/KittyswapsExchange.sol) methods to call. How to build and encode the `_data` payload for the respective methods is explained in the [Data Encoding](#data-encoding) section. 

## Exchanging Tokens

In `KittyswapsExchange.sol`, there are two methods for exchanging tokens:
...
In `KittyswapsExchange.sol`, there are two methods for managing token reserves supplies:
...
In Kittyswaps, like Uniswap, the price of an asset is a function of a currency reserve and the corresponding token reserve. Indeed, all methods in Kittyswaps enforce that the the following equality remains true: 
...
The currency is an ERC-1155 token that is fungible (>0 decimals) that is used to price each token $i$ in a given ERC-1155 token contract. For instance, this currency could be wrapped Ether or wrapped DAI (see [erc20-meta-wrapper](https://github.com/kittyswaps/erc20-meta-wrapper)). Since the currency is an ERC-1155, the KittyswapsExchange.sol contract also needs to be aware of what the currency `id` is in the currency contract. The currency can be the same contract as the Tokens contract.
...
It is possible to buy/sell multiple tokens at once, but if any one fails, the entire trade will fail as well. This could change for Kittyswaps V2.
...
Since users can't know exactly how much currency will be required when the transaction is created, they must provide a `_maxCurrency` value which contain the maximum amount of currency they are willing to spend for the entire trade. It would've been possible for Kittyswaps to support a maximum amount per token $i$, however this would increase the gas cost significantly. If proven to be desired, this could be incorporated in Kittyswaps V2.

Additionally, to protect users against miners or third party relayers withholding their Kittyswaps trade transactions, a `_deadline` parameter must be provided by the user. This `_deadline`is a block number after which a given transaction will revert.

Finally, users can specify who should receive the tokens with the `_recipient` argument. This is particularly useful for third parties and proxy contracts that will interact with Kittyswaps.

The `_maxCurrency` argument is specified as the amount of currency sent to the KittyswapsExchange.sol contract via the `onERC1155BatchReceived()` method :

```solidity
// Tokens received need to be currency contract
require(msg.sender == address(currency), "KittyswapsExchange#onERC1155BatchReceived: INVALID_CURRENCY_TRANSFERRED");
require(_ids.length == 1, "KittyswapsExchange#onERC1155BatchReceived: INVALID_CURRENCY_IDS_AMOUNT");
require(_ids[0] == currencyID, "KittyswapsExchange#onERC1155BatchReceived: INVALID_CURRENCY_ID");
...
To call this method, users must transfer sufficient currency to the KittyswapsExchange.sol, as follow:

```solidity
// Call _currencyToToken() on KittyswapsExchange.sol contract
IERC1155(CurrencyContract).safeTranferFrom(_from, kittyswaps_address, curency_id, _maxCurrency, _data);
```
...
Since users can't know exactly how much currency they would receive when the transaction is created, they must provide a `_minCurrency` value which contain the minimum amount of currency they are willing to accept for the entire trade.  It would've been possible for Kittyswaps to support a minimum amount per token $i$, however this would increase the gas cost significantly. If proven to be desired, this could be incorporated in Kittyswaps V2.

Additionally, to protect users against miners or third party relayers withholding their Kittyswaps trade transactions, a `_deadline` parameter must be provided by the user. This `_deadline`is a block number after which a given transaction will revert.

Finally, users can specify who should receive the currency with the `_recipient` argument upon the completion of the trade. This is particularly useful for third parties and proxy contracts that will interact with Kittyswaps. 

The `_tokenIds` and  `_tokensSoldAmounts` arguments are specified as the token ids and token amounts sent to the KittyswapsExchange.sol contract via the `onERC1155BatchReceived()` method :

```solidity
// Tokens received need to be correct ERC-1155 Token contract
require(msg.sender == address(token), "KittyswapsExchange#onERC1155BatchReceived: INVALID_TOKENS_TRANSFERRED");
...
To call this method, users must transfer the tokens to sell to the KittyswapsExchange.sol contract, as follow:

```solidity
// Call _tokenToCurrency() on KittyswapsExchange.sol contract
IERC1155(TokenContract).safeBatchTranferFrom(_from, kittyswaps_address, _ids, _amounts, _data);
```
...
The `_provider` argument is the address of who sent the tokens and the `_tokenIds` and  `_tokenAmounts` arguments are specified as the token ids and token amounts sent to the KittyswapsExchange.sol contract via the `onERC1155BatchReceived()` method:

```solidity
// Tokens received need to be correct ERC-1155 Token contract
require(msg.sender == address(token), "KittyswapsExchange#onERC1155BatchReceived: INVALID_TOKEN_TRANSFERRED");
...
To call this method, users must transfer the tokens to add to the KittyswapsExchange.sol liquidity pools, as follow:

```solidity
// Call _addLiquidity() on KittyswapsExchange.sol contract
IERC1155(TokenContract).safeBatchTranferFrom(_provider, kittyswaps_address, _ids, _amounts, _data);
```
...
The `_provider` argument is the address of who sent the liquidity pool tokens, the `_tokenIds` and `_poolTokenAmounts` arguments are specified as the token ids and liquidity pool token amounts sent to the KittyswapsExchange.sol contract via the `onERC1155BatchReceived()` method:

```solidity
// Tokens received need to be NIFTY-1155 tokens (liquidity pool tokens)
require(msg.sender == address(this), "KittyswapsExchange#onERC1155BatchReceived: INVALID_NIFTY_TOKENS_TRANSFERRED");
...
To call this method, users must transfer the liquidity pool tokens to burn to the KittyswapsExchange.sol contract, as follow:

```solidity
// Call _removeLiquidity() on KittyswapsExchange.sol contract
IERC1155(KittyswapsExchange).safeBatchTranferFrom(_provider, kittyswaps_address, _ids, _amounts, _data);
```
...
There methods are useful for clients and third parties to query the current state of a KittyswapsExchange.sol contract.
...
Three main functions in KittyswapsExchange.sol are subjected to rounding errors: `_addLiquidity()`, `_currencyToToken()` and `_tokenToCurrency()`. 
