 ...
 *
 * See https://github.com/kittyswaps/erc20-meta-token for a generalized
 ...
 */
contract KittyswapsExchange is ReentrancyGuard, ERC1155MintBurn, ERC1155Meta {
  ...
  constructor(address _tokenAddr, address _currencyAddr, uint256 _currencyID) public {
    require(
      ...
      "KittyswapsExchange#constructor:INVALID_INPUT"
    );
    factory = msg.sender;
  ...
  {
    // Input validation
    require(_deadline >= block.timestamp, "KittyswapsExchange#_currencyToToken: DEADLINE_EXCEEDED");
    ...
    for (uint256 i = 0; i < nTokens; i++) {
      ...
      require(amountBought > 0, "KittyswapsExchange#_currencyToToken: NULL_TOKENS_BOUGHT");
   ...
   * @return price Amount of currency tokens to send to Kittyswaps.
   */
    ...
    public pure returns (uint256 price)
  {
    ...
    require(_assetSoldReserve > 0 && _assetBoughtReserve > 0, "KittyswapsExchange#getBuyPrice: EMPTY_RESERVE");
    ...
    internal nonReentrant() returns (uint256[] memory currencyBought)
  {
    ...
    // Input validation
    require(_deadline >= block.timestamp, "KittyswapsExchange#_tokenToCurrency: DEADLINE_EXCEEDED");
    ...
    for (uint256 i = 0; i < nTokens; i++) {
      ...
      require(amountSold > 0, "KittyswapsExchange#_tokenToCurrency: NULL_TOKENS_SOLD");
    ...
    require(totalCurrency >= _minCurrency, "KittyswapsExchange#_tokenToCurrency: INSUFFICIENT_CURRENCY_AMOUNT");
   ...
   * @return price Amount of currency tokens to receive from Kittyswaps.
   */
  ...
  {
    //Reserves must not be empty
    require(_assetSoldReserve > 0 && _assetBoughtReserve > 0, "KittyswapsExchange#getSellPrice: EMPTY_RESERVE");
    ...
    return numerator / denominator; //Rounding errors will favor Kittyswaps, so nothing to do
  }
  ...
  {
    // Requirements
    require(_deadline >= block.timestamp, "KittyswapsExchange#_addLiquidity: DEADLINE_EXCEEDED");
    ...
    for (uint256 i = 0; i < nTokens; i ++) {
      ...
      require(_maxCurrency[i] > 0, "KittyswapsExchange#_addLiquidity: NULL_MAX_CURRENCY");
      require(amount > 0, "KittyswapsExchange#_addLiquidity: NULL_TOKENS_AMOUNT");

      // If the token contract and currency contract are the same, prevent the creation
      // of a currency pool.
      if (currencyPoolBanned) {
        require(tokenId != currencyID, "KittyswapsExchange#_addLiquidity: CURRENCY_POOL_FORBIDDEN");
      }
      ...
      if (totalLiquidity > 0) {
        ...
        */
        ...
        require(_maxCurrency[i] >= currencyAmount, "KittyswapsExchange#_addLiquidity: MAX_CURRENCY_AMOUNT_EXCEEDED");
        ...
        // See https://github.com/kittyswaps/kittyswaps/issues/19
        ...
        require(maxCurrency >= 1000000000, "KittyswapsExchange#_addLiquidity: INVALID_CURRENCY_AMOUNT");
  ...
  {
    // Input validation
    require(_deadline > block.timestamp, "KittyswapsExchange#_removeLiquidity: DEADLINE_EXCEEDED");
    ...
    for (uint256 i = 0; i < nTokens; i++) {
      ...
      require(totalLiquidity > 0, "KittyswapsExchange#_removeLiquidity: NULL_TOTAL_LIQUIDITY");
      ...
      require(currencyAmount >= _minCurrency[i], "KittyswapsExchange#_removeLiquidity: INSUFFICIENT_CURRENCY_AMOUNT");
      require(tokenAmount >= _minTokens[i], "KittyswapsExchange#_removeLiquidity: INSUFFICIENT_TOKENS");
    ...
    public returns(bytes4)
  {
    ...
    // Users must be responsible and only use this Kittyswaps exchange
    ...
    |__________________________________*/
      ...
      require(msg.sender == address(currency), "KittyswapsExchange#onERC1155BatchReceived: INVALID_TOKENS_DEPOSITED");
      require(_ids[0] == currencyID, "KittyswapsExchange#onERC1155BatchReceived: INVALID_CURRENCY_ID");

    } else {
      revert("KittyswapsExchange#onERC1155BatchReceived: INVALID_METHOD");
    }
    ...
    public returns(bytes4)
  {
    ...
    require(
      ...
      "KittyswapsExchange#onERC1155Received: INVALID_ONRECEIVED_MESSAGE"
    );
   ...
   */
  fallback () external {
    revert("KittyswapsExchange:UNSUPPORTED_METHOD");
  }
    ...
    internal view returns (uint256[] memory)
  {
    ...
    if (nTokens == 1) {
      ...
      for (uint256 i = 1; i < nTokens; i++) {
        require(_tokenIds[i-1] < _tokenIds[i], "KittyswapsExchange#_getTokenReserves: UNSORTED_OR_DUPLICATE_TOKEN_IDS");
