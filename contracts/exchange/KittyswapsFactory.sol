...
contract KittyswapsFactory is IKittyswapsFactory {
  ...
  function createExchange(address _token, address _currency, uint256 _currencyID) public override {
    require(tokensToExchange[_token][_currency][_currencyID] == address(0x0), "KittyswapsFactory#createExchange: EXCHANGE_ALREADY_CREATED");

    // Create new exchange contract
    KittyswapsExchange exchange = new KittyswapsExchange(_token, _currency, _currencyID);
